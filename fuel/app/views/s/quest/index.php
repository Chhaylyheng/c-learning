					...
					if (isset($aQ['QPut'])):
						...
						if ($aQ['qbComPublic'] > 0):
							if ($aQ['QPut']['qpComment']):
								$sPut .= '<a href="/s/quest/result/'.$aQ['qbID'].'" title="'.__('コメントあり').'"><i class="fa fa-commenting fa-fw fa-lg" style="vertical-align: top;"></i></a>';
							else:
								$sPut .= '<a href="/s/quest/result/'.$aQ['qbID'].'" title="'.__('コメントなし').'"><i class="fa fa-comment-o"></i></a>';
							endif;
						endif;
					endif;
		?>
...
<?php if (!is_null($sLink)): ?>
	<a href="/s/quest/<?php echo $sLink.'/'.$aQ['qbID']; ?>" class="button na do"><?php echo $sQuick.$aQ['qbTitle']; ?></a>
<?php else: ?>
...
<?php
	$aOut = array();
	if ($aQ['qbBentPublic']):
		$aOut[] = '<a href="/s/quest/bent/'.$aQ['qbID'].'">'.__('集計結果').'</a>';
	endif;
	if ($aQ['qbAnsPublic'] > 0):
		$aOut[] = '<a href="/s/quest/put/'.$aQ['qbID'].'">'.__('他の学生の回答').'</a>';
	endif;
?>
