...
foreach ($aContact as $iP => $aThread):
	...
	$sDelDisp = ($aP['coID'] == $aTeacher['ttID'])? 'inline-block':'none';
	$sSubject = ($aP['coSubject'])? $aP['coSubject']:'(No subject)';

	$aRead = array('icon'=>'fa-envelope-open-o', 'color'=>($iResNum || $aP['coTeach'])? '#ffffff':'#76AEDC', 'new'=>'');
...
<?php
	if (isset($aThread['C'])):
		...
		foreach ($aThread['C'] as $iC => $aC):
			...
			$sJsKey = $iP.'_'.$iC;
			$sDelDisp = ($aC['coID'] == $aTeacher['ttID'])? 'inline-block':'none';
...
<form action="/t/contact/res" method="post" class="res-box width-100" style="display: none;">
	<input type="hidden" name="c_st" value="<?php echo (isset($aStu))? $aStu['stID']:''; ?>">
	...
	<div class="formControl font-size-90 width-100" style="margin: auto;">
		...
		<div class="formGroup width-100">
			...
			<div class="formContent inline-box">
				...
				<p class="note mt8"><?php echo __('※学生宛にメールが送信されます。'); ?></p>
			</div>
		</div>
	</div>
	<div class="res-button-box">
		<button type="submit" class="button do na width-auto ContactReplyToSubmit font-size-90 ToContact" style="padding: 4px 8px; display: none;"><?php echo __('連絡する'); ?></button>
