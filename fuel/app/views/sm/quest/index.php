			...
			if (isset($aQ['QPut'])):
				...
				if ($aQ['qbComPublic'] > 0):
					if ($aQ['QPut']['qpComment']):
						$sPut .= '<a href="/s/quest/result/'.$aQ['qbID'].Clfunc_Mobile::SesID().'">'.Clfunc_Mobile::emj('SMILE').'</a>';
					endif;
				endif;
			endif;
?>
...
<?php if (!is_null($sLink)): ?>
	<?php echo Clfunc_Mobile::emj('PENCIL'); ?><a href="/s/quest/<?php echo $sLink.DS.$aQ['qbID'].Clfunc_Mobile::SesID(); ?>"><?php echo $sQuick.$aQ['qbTitle']; ?></a><br>
<?php else: ?>
...
<?php
	...
	$sSep = '┗';
	if ($aQ['qbAnsPublic'] > 0):
		$sAns = '┗<a href="/s/quest/put/'.$aQ['qbID'].Clfunc_Mobile::SesID().'">'.__('他の学生の回答').'</a><br>';
		$sSep = '┣';
	endif;
	if ($aQ['qbBentPublic']):
		$sBent = $sSep.'<a href="/s/quest/bent/'.$aQ['qbID'].Clfunc_Mobile::SesID().'">'.__('集計結果').'</a><br>';
		$sSep = '┣';
	endif;
?>
